...
class BodyPlane(object):
    """Create torso plane from given points
    ...
    def __init__(self, points, **kwargs):
        if len(points) == 1 and 'normal_vector' in kwargs:
            ...
            self.p1 = np.array(points[0])
            self.normal_vector = BodyPlane.calculate_normal_vector(points)
        elif len(points) > 3:
    ...
    def calculate_normal_vector(points):
        ...
        '''
        if BodyPlane.are_collinear(points):
