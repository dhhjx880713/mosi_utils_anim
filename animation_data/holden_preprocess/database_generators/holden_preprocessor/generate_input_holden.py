...
masks = ["%s/LocomotionFlat01_000%s%s.bvh",
		 "%s/LocomotionFlat02_000%s%s.bvh", "%s/LocomotionFlat02_001%s%s.bvh", "%s/LocomotionFlat03_000%s%s.bvh",
		 "%s/LocomotionFlat04_000%s%s.bvh", "%s/LocomotionFlat05_000%s%s.bvh", "%s/LocomotionFlat06_001%s%s.bvh", "%s/LocomotionFlat07_000%s%s.bvh",
		 "%s/LocomotionFlat08_001%s%s.bvh", "%s/LocomotionFlat09_000%s%s.bvh", "%s/LocomotionFlat10_000%s%s.bvh", "%s/LocomotionFlat11_000%s%s.bvh",
		 #"%s/LocomotionFlat12_000%s%s.bvh" # includes crouching
		]


class Participant():
	def __init__(self, path, label_path, style, style_id):
		self.style = style
		self.path, self.style_id = path, style_id
		...
		Xpart, Ypart, Ppart = self.load_data_masks(masks, mirrored="_mirror")
	...
	def load_data_masks(self, mask_, mirrored = ""):
		Ppart, Xpart, Ypart = [], [], []
		for m in mask_:
			data = m%(self.path, mirrored, "" if self.style == "" else ("_"+self.style))
			label_data = m%(self.label_path, mirrored, "")
			print("processing: ", data, label_data)
			...
			self.phase = np.loadtxt(label_data.replace('.bvh', '.phase'))[::2]
			gait = np.loadtxt(label_data.replace('.bvh', '.gait'))[::2]
			self.gait = np.concatenate([
				gait[:, 0:1],  # standing
				gait[:, 1:2],  # walking
				gait[:, 2:3] + gait[:, 3:4],  # jogging + running
			], axis=-1)
			if np.any(gait[:, 5:]):
				print("gait > 0")
	...
	def generate_train_test(self, rng, test_prop = 0.2, randomized_indices_file = False):
		if not randomized_indices_file:
			...
			np.save(self.path + "/randomization.npy", indices)
		else:
			indices = np.load(self.path + "/randomization.npy")
			print("indices loaded from ", self.path + "/randomization.npy", indices)
	...
	def __process_data(self):
		...
		# holden end_joints:
		end_joints = [5, 10, 16, 22, 23, 29, 30]
		...
		sdr_l, sdr_r, hip_r, hip_l = 18, 25, 2, 7
		across = (
