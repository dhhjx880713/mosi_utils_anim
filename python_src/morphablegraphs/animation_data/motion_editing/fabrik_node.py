import numpy as np
from .fabrik_chain import FABRIKChain, to_local_cos, quaternion_from_vector_to_vector


def get_joint_parameters_from_positions(skeleton, positions):
    n_parameters = len(skeleton.animated_joints)*4+3
    frame = np.zeros(n_parameters)
    o = 3
    for node in skeleton.animated_joints:
        child_node = skeleton.nodes[node].children[0]
        child_offset = np.array(child_node.offset)
        child_offset /= np.linalg.norm(child_offset)
        if child_node.node_name in positions:
            dir_to_child = positions[child_node.node_name] - positions[node]
            dir_to_child /= np.linalg.norm(dir_to_child)
            global_q = quaternion_from_vector_to_vector(child_offset, dir_to_child)
...
class FABRIKNode(object):
    def __init__(self, skeleton, root, parent_chain=None, tolerance=0.01, max_iter=500):
        ...
        for c in children:
            ...
            while current_node is not None:
                ...
                if n_children == 1:  # append to chain
                    ...
                    effectors.append(current_node)
                    chain = FABRIKChain(skeleton, joints, effectors)
                    node = FABRIKNode(skeleton, current_node, chain, tolerance, max_iter)
                    self.child_nodes.append(node)
                    current_node = None

    def construct_from_skeleton2(self, skeleton, root, tolerance, max_iter):
        ...
        for c in children:
            ...
            while current_node is not None:
                ...
                if n_children == 1:  # append to chain
                    ...
                    if n_children > 0:
                        joints.append(current_node)
                    parent_chain = FABRIKChain(skeleton, joints, joints)
                    print("construct node at", current_node)
    ...
    def forward_stage(self):
        ...
        for c in self.child_nodes:
            if self.parent_chain is not None:
                c.parent_chain.root_pos = self.parent_chain.positions[-1]
            else:
                c.parent_chain.root_pos = self.root_pos
            c.forward_stage()

    def get_chain_root_position(self):
        return self.parent_chain.positions[0]
    ...
    def print_end_effectors(self):
        ...
        for c in self.child_nodes:
            if c.is_leaf:
                print(c.root, c.parent_chain.target, c.parent_chain.positions[-1])
            else:
    ...
    def get_joint_parameters(self, frame):
        ...
        for c in self.child_nodes:
            #print("from", c.parent_chain.joints[0], "to", c.parent_chain.joints[-1])
            global_frame = c.parent_chain.get_joint_parameters_global()
            src = 3
            if self.parent_chain is None:
                animated_joints = c.parent_chain.joints
            else:
                animated_joints = c.parent_chain.joints[1:]
            for j in animated_joints: #ignore root rotation
                dest = self.skeleton.animated_joints.index(j)*4 +3 # is this correct?
    ...
    def set_targets(self, targets):
        ...
        if self.is_leaf:
            if self.root in targets:
                ...
                self.target = self.parent_chain.positions[-1] # keep the initial position as target for unconstrained leafs
                self.parent_chain.target = self.target
        else:
            for c in self.child_nodes:
    ...
    def run(self, orig_frame, targets):
        ...
        if False:
            ...
            frame = get_joint_parameters_from_positions(self.skeleton, joint_pos)
