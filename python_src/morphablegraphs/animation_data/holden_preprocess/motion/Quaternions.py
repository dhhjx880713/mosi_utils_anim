...
class Quaternions:
	...
	def sizeSquare(self):
		return self.qs[0][0] * self.qs[0][0] +self.qs[0][1] * self.qs[0][1] + self.qs[0][2] * self.qs[0][2] + self.qs[0][3] * self.qs[0][3]

	def inverted(self):
		return Quaternions(np.array([self.qs[0][0], -self.qs[0][1], -self.qs[0][2], -self.qs[0][3]]))

	def normalize(self):
		square_sum = np.sum(self.qs[0] ** 2)
		if square_sum >= 1.e-8:
			scale = 1 / math.sqrt(square_sum)

			self.qs[0] *= scale
		else:
			self.qs[0][0] = 1.0
			self.qs[0][1:] = 0.0
		return Quaternions(self)



	def swing_twist_decomposition(self, twist_axis):
		q = self.normalize()
		twist_axis /= np.sqrt(np.sum(twist_axis**2))

		#twist_axis = np.array((q * offset))[0]
		projection = np.dot(twist_axis, np.array([q.qs[0][1], q.qs[0][2], q.qs[0][3]])) * twist_axis
		twist = Quaternions(np.array([q.qs[0][0], projection[0], projection[1],projection[2]], dtype=float))
		if (twist.sizeSquare() == 0):
			twist = Quaternions(np.array([[1.0,0.0,0.0,0.0]]))
...
q = Quaternions(np.array([[1.0,0.0,0.0,0.0]]))
q.normalize()
q.inverted()