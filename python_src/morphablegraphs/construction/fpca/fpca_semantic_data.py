...
class FPCASemanticData():
    ...
    def spline_representation(self):
        ...
        for item in self.annotation_data.keys():
            filename = os.path.split(item)[-1]
            self.semantic_spline[filename] = []
            for feature, anno_vec in self.annotation_data[item].iteritems():
                # print(range(len(anno_vec)))
                # print(anno_vec)
                # print(self.knots)
                knots, coeffs, degree = si.splrep(range(len(anno_vec)), anno_vec, t=self.knots[4: -4])
                self.semantic_spline[filename] += coeffs[:-4].tolist()

    def dimension_reduction(self):
        self.filename_order = self.semantic_spline.keys()
        self.semantic_vecs = np.asarray(self.semantic_spline.values())
