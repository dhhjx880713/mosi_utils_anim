...
class FPCATemporalData(object):
    ...
    def _z_t_transform(self):
        ...
        for filename in self.temporal_data:
            ...
            assert self._is_strict_incresing(tmp), ("convert %s to monotonic indices failed" % filename)
            w_tmp = np.array(tmp)
    ...
    def fpca_on_temporal_data(self):
        ...
        scores = np.asarray(self.temporal_pcaobj[self.temporal_pca.names.index('scores')])
