...
@author: mamanns, mamauer, Erik Herrmann
...
#        weights = [np.exp(max_level - bvh_reader.node_levels[node_name]/max_level) for node_name in node_name_map.keys()]
        weights = [np.exp(-bvh_reader.node_levels[node_name]) for node_name in node_name_map.keys()]
    else:
#        weights = [np.exp(max_level - bvh_reader.node_levels[node_name]/max_level) for node_name in bvh_reader.node_names.keys()]
        weights = [np.exp(-bvh_reader.node_levels[node_name]) for node_name in bvh_reader.node_names.keys()]
    return weights
...
class BVHReader(BVHR):
    ...
    def __init__(self, filename):
        ...
        self.node_names, self.node_levels = self._get_node_names()
        self.max_level = max(self.node_levels.values())
        self.parent_dict = self._get_parent_dict()
        

    def _get_node_names(self):
        """Returns an OrderedDict from node name to node object. This method ignores end sites"""

        
        def get_node_names_nodes(node, level):
            """Returns list of  2-tuples (nodename, node)"""
            
            nnlist = []
	     #End nodes need to be ignored otherwise the ordered dictionary 
            #can not be used to get the index for the keyframe values.
            if not node.isEndSite():    
                nnlist.append((node.name, node, level))
            level += 1
            for childnode in node.children:
                nnlist += get_node_names_nodes(childnode, level)
            level -= 1
            return nnlist
        level = 0
        return OrderedDict([(t[0],t[1]) for t in get_node_names_nodes(self.root,level)]),\
               OrderedDict([(t[0],t[2]) for t in get_node_names_nodes(self.root,level)])
    ...
    def onMotion(self,frames,dt):
        """Called for frame time and number of frames. Must be subclassed"""
...
class BVHWriter(object):
    ...
    def generate_bvh_string(self):
          bvh_string = self._generate_hierarchy_string(self.bvh_reader.root)+"\n"
    ...
    def _generate_hierarchy_string(self,root):
        ...
        hierarchy_string += self._generate_joint_string(root,0)
        return hierarchy_string
     
          
    def _generate_joint_string(self,joint, joint_level):
        ...
        if joint_level == 0:
            joint_string +=tab_string+ "ROOT "+joint.name+"\n"
        else:
            if joint.name != "End Site" and joint.channels >0:
                joint_string +=tab_string+ "JOINT "+joint.name+"\n"
            else:
        ...
        joint_string += tab_string + "\t"  + "OFFSET " +"\t "+ \
                str(joint.offset[0]) +"\t "+str(joint.offset[1]) \
                +"\t "+str(joint.offset[2])+"\n"
                
        if joint.name != "End Site":
            #channel information
            joint_string += tab_string +"\t"  + "CHANNELS "+str(len(joint.channels))+" "
            for tok in joint.channels:
                joint_string += tok+" "
            joint_string+="\n"
            ...
            for child in joint.children:
                joint_string += self._generate_joint_string(child,joint_level)
    ...
    def _generate_frame_parameter_string(self,frame_data,node_names,frame_time ,is_quaternion = False):
        ...
        if not is_quaternion:
            skip_joints = True
            if len(frame_data[0]) == len([n for n in node_names if not node_names[n].isEndSite() ])*3 +3:
                skip_joints = False
            if not skip_joints:
                ...
                for frame in frame_data:
                      ...
                      for node_name in node_names:  # go through the node names to
                        ...
                        if not node_names[node_name].isEndSite(): #ignore end sites completely
                            if not node_name.startswith("Bip") or not skip_joints:
                                if node_name in ["Bip01_R_Toe0","Bip01_L_Toe0"]:
                                    ...
                                    if node_names[node_name].isRoot():
            ...
            skip_joints = True
            if len(frame_data[0]) == len([n for n in node_names if not node_names[n].isEndSite() ])*4 +3:
                skip_joints = False
            euler_frames = []
            for frame in frame_data:
...
def main():
    ...
    BVHWriter(out_file,reader,motion_sample,frame_time=reader.frame_time,\
                                        is_quaternion = True)
