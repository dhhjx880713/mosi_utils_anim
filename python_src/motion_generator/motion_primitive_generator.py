...
class MotionPrimitiveGenerator(object):
    ...
    * prev_action_name : string
    ...
    def __init__(self, action_constraints, algorithm_config, prev_action_name=""):
        ...
        self._constrained_gmm_config = self._algorithm_config[
            "constrained_gmm_settings"]
        self._optimization_settings = self._algorithm_config[
        ...
        self.use_constrained_gmm = self._algorithm_config[
            "use_constrained_gmm"]
        self.use_transition_model = self._algorithm_config[
            "use_transition_model"]
        self.activate_cluster_search = self._algorithm_config[
            "activate_cluster_search"]
        self.activate_parameter_check = self._algorithm_config[
    ...
    def generate_motion_primitive_from_constraints(self, motion_primitive_constraints, prev_motion):
        ...
        * parameters : np.ndarray
        ...
        tmp_quat_frames = self._morphable_graph.subgraphs[self.action_name].nodes[
            mp_name].motion_primitive.back_project(parameters, use_time_parameters=True).get_motion_vector()
    ...
    def get_optimal_motion_primitive_parameters(self, mp_name,
        ...
        * parameters : np.ndarray
        ...
        # estimate parameters fitting constraints
        if self.use_constraints and len(constraints) > 0:
            graph_node = self._morphable_graph.subgraphs[
                self.action_name].nodes[mp_name]

            # A) find best sample from model
            if self.activate_cluster_search:
                #  find best sample using a directed search in a
                #  space partitioning data structure
                parameters = self._search_for_best_sample(
                    graph_node, constraints, prev_frames, self._algorithm_config["n_cluster_search_candidates"])
            ...
            # B) optimize sampled parameters as initial guess if the
            # constraints were not reached
            if not self.use_transition_model and use_optimization and not close_to_optimum:
                bounding_boxes = (
                    graph_node.parameter_bb, graph_node.cartesian_bb)
    ...
    def _search_for_best_sample(self, graph_node, constraints, prev_frames, n_candidates=2):
        ...
        distance, s = graph_node.search_best_sample(
            obj_error_sum, data, n_candidates)
    ...
    def _get_random_parameters(self, mp_name, prev_mp_name="",
        ...
        if self.use_transition_model and prev_parameters is not None and self._morphable_graph.subgraphs[self.prev_action_name].nodes[prev_mp_name].has_transition_model(to_key):
            ...
            parameters = self._morphable_graph.subgraphs[self.prev_action_name].nodes[
                prev_mp_name].predict_parameters(to_key, prev_parameters)
        else:
            parameters = self._morphable_graph.subgraphs[
                self.action_name].nodes[mp_name].sample_parameters()
        return parameters

    def _predict_gmm(self, mp_name, prev_mp_name, prev_frames, prev_parameters):
        to_key = self.action_name + "_" + mp_name
        gmm = self._morphable_graph.subgraphs[self.prev_action_name].nodes[
            prev_mp_name].predict_gmm(to_key, prev_parameters)
        return gmm
    ...
    def _sample_and_pick_best(self, mp_node, gmm, constraints, prev_frames):
        ...
        * success : bool
        ...
        """
        samples, distances, successes = self._sample_from_gmm(
            mp_node, gmm, constraints, prev_frames)
        ...
        * successes : list of bool 
        ...
        while count < self.n_random_samples:
            ...
            if valid:
                ...
                acr_length = mp_node.get_step_length_for_sample(
                    s, method="arc_length")
                absolute_length = mp_node.get_step_length_for_sample(
                    s, method="distance")
