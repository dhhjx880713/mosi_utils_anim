...
LOG_FILE = "log.txt"
# TODO replace with standard skeleton in data directory
    ...
    * use_transition_model : Booelan
        ...
        * motion : AnnotatedMotion
        ...
        if export:
            ...
            if motion.quat_frames is not None:
                ...
                write_to_logfile(self._service_config[
                                 "output_dir"] + os.sep + LOG_FILE, prefix, self._algorithm_config)
                self.export_synthesis_result(mg_input, self._service_config[
                                             "output_dir"], output_filename, motion, add_time_stamp=True)
            else:
    ...
    def _generate_motion_from_constraints(self, motion_constraints):
        ...
        * motion: AnnotatedMotion
        ...
        while action_constraints is not None:
            ...
            success = self._append_elementary_action_to_motion(
                action_constraints, motion)
    ...
    def _append_elementary_action_to_motion(self, action_constraints, motion):
        ...
        * motion: AnnotatedMotion
        ...
        # TODO move trajectory_following_settings to different key of the
        # algorithm_config
        trajectory_following_settings = self._algorithm_config[
            "trajectory_following_settings"]

        motion_primitive_constraints_builder = MotionPrimitiveConstraintsBuilder()
        motion_primitive_constraints_builder.set_action_constraints(
            action_constraints)
        motion_primitive_constraints_builder.set_trajectory_following_settings(
            trajectory_following_settings)
        motion_primitive_generator = MotionPrimitiveGenerator(
            action_constraints, self._algorithm_config, prev_action_name)
        ...
        arc_length_of_end = morphable_subgraph.nodes[
            morphable_subgraph.get_random_end_state()].average_step_length
    ...
    #
        # create sequence of list motion primitives,arc length and number of
        # frames for backstepping
        ...
        while current_motion_primitive_type != NODE_TYPE_END:
            ...
            if self._algorithm_config["debug_max_step"] > -1 and motion.step_count + temp_step > self._algorithm_config["debug_max_step"]:
                ...
                break
            ###################################################################
            # Get motion primitive = extract from graph based on previous last
            # step + heuristic
            if temp_step == 0:
                current_motion_primitive = action_constraints.parent_constraint.morphable_graph.get_random_action_transition(
                    motion, action_constraints.action_name)
                ...
                current_motion_primitive, current_motion_primitive_type = morphable_subgraph.get_random_transition(
                    motion, action_constraints, travelled_arc_length, arc_length_of_end)
                if current_motion_primitive is None:
            ...
            try:
                ...
                motion_primitive_constraints_builder.set_status(
                    current_motion_primitive, travelled_arc_length, motion.quat_frames, is_last_step)
            ...
            tmp_quat_frames, parameters = motion_primitive_generator.generate_motion_primitive_from_constraints(
                motion_primitive_constraints, motion)

            # update annotated motion
            canonical_keyframe_labels = morphable_subgraph.get_canonical_keyframe_labels(
                current_motion_primitive)
            start_frame = motion.n_frames
            motion.append_quat_frames(tmp_quat_frames)
            last_frame = motion.n_frames - 1
            motion.update_action_list(motion_primitive_constraints.constraints,
                                      action_constraints.keyframe_annotations, canonical_keyframe_labels, start_frame, last_frame)
            ...
            if action_constraints.trajectory is not None:
                if len(motion.graph_walk) > 0:
                    ...
                    min_arc_length = 0.0
                closest_point, distance = action_constraints.trajectory.find_closest_point(
                    motion.quat_frames[-1][:3], min_arc_length=min_arc_length)
                travelled_arc_length, eval_point = action_constraints.trajectory.get_absolute_arc_length_of_point(
                    closest_point, min_arc_length=travelled_arc_length)
            ...
            graph_walk_entry = GraphWalkEntry(
                action_constraints.action_name, current_motion_primitive, parameters, travelled_arc_length)
        ...
        motion.update_frame_annotation(
            action_constraints.action_name, start_frame, motion.n_frames)
...
#         return
    ...
    def print_runtime_statistics(self, time_in_seconds):
        ...
        total_time_string = "finished synthesis in " + \
            str(minutes) + " minutes " + str(seconds) + " seconds"
        evaluations_string = "total number of objective evaluations " + \
            str(global_counter_dict["evaluations"])
        error_string = "average error for " + str(len(global_counter_dict["motionPrimitveErrors"])) + " motion primitives: " + str(
            np.average(global_counter_dict["motionPrimitveErrors"], axis=0))
    ...
    def export_synthesis_result(self, input_data, output_dir, output_filename, motion, add_time_stamp=False):
        ...
        """
        write_to_json_file(
            output_dir + os.sep + output_filename + ".json", input_data)
        write_to_json_file(
            output_dir + os.sep + output_filename + "_actions" + ".json", motion.action_list)
        ...
        write_to_json_file(output_dir + os.sep + output_filename +
                           "_annotations" + ".json", reordered_frame_annotation)
        export_quat_frames_to_bvh_file(output_dir, self.morphable_graph.skeleton,
                                       motion.quat_frames, prefix=output_filename, start_pose=None, time_stamp=add_time_stamp)
