...
class AnnotatedMotion(object):
    ...
    * start_pose: dict
    ...
    def update_frame_annotation(self, action_name, start_frame, end_frame):
        ...
        self.frame_annotation['elementaryActionSequence'].append(
            action_frame_annotation)
    ...
    def update_action_list(self, constraints, keyframe_annotations, canonical_key_frame_annotation, start_frame, last_frame):
        ...
        """
        new_action_list = self._associate_actions_to_frames(
            self.quat_frames, canonical_key_frame_annotation, constraints, keyframe_annotations, start_frame, last_frame)
    ...
    def _associate_actions_to_frames(self, quat_frames, time_information, constraints, keyframe_annotations, start_frame, last_frame):
        ...
        *  action_list : dict of lists of dicts
        ...
        # extract the set of keyframes and their annotations referred to by the
        # constraints
        for c in constraints:
            if "semanticAnnotation" in c.keys():
                # can also contain lastFrame and firstFrame
                for key_label in c["semanticAnnotation"]:
        ...
        for key_frame, key_label in key_frame_label_pairs:
            ...
            if num_events > 1:
                ...
                event_list = [(annotations[i]["event"], annotations[i])
                              for i in xrange(num_events)]
                temp_event_dict = dict()
                # merge parameters to one event if it is found multiple times
                for name, event in event_list:
                    if name not in temp_event_dict.keys():
                        ...
                        if "joint" in temp_event_dict[name]["parameters"].keys():
                            existing_entry = copy(
                                temp_event_dict[name]["parameters"]["joint"])
                            if isinstance(existing_entry, basestring):
                                temp_event_dict[name]["parameters"]["joint"] = [
                                    existing_entry, event["parameters"]["joint"]]
                            else:
                                temp_event_dict[name]["parameters"][
                                    "joint"].append(event["parameters"]["joint"])
