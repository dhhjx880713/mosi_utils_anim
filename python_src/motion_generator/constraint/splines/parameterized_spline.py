...
def get_closest_lower_value(arr, left, right, value, getter=lambda A, i: A[i]):
    ...
    # print delta
    # or (left ==0 and (delta> 0) ):# or (right == len(A)-1 and ()):#test if
    # there are more than two elements to explore
    if (delta > 1):
        ...
        if test_value > value:
            ...
            # print "done"
            return (i_mid, 0)
    # always return the lowest closest value if no value was found, see flags
    # for the cases
    else:
    ...
    def get_full_arc_length(self, granularity=1000):
        ...
        for accumulated_step in accumulated_steps:
            ...
            if point is not None:
                ...
                raise ValueError(
                    'queried point is None at %f' % (accumulated_step))
...
#         i = min(i,self.number_of_segments)
        # the part of t before i
        index = min(
            floor(self.number_of_segments * u), self.number_of_segments)
        # the rest, e.g. N = 10 and t = 0.62 => i = 6 and the rest is 0.02
        local_u = (self.number_of_segments * u) - \
            floor(self.number_of_segments * u)
        #i = min(i,self.number_of_segments)
    ...
    def _find_closest_values_in_relative_arc_length_map(self, relative_arc_length):
        ...
        result = get_closest_lower_value(self._relative_arc_length_map, 0,
                                         len(self._relative_arc_length_map) -
                                         1,
                                         relative_arc_length,
        ...
        if result[1] == 0:  # found exact value
            ...
            # check array bounds
            if index < len(self._relative_arc_length_map):
                ...
                ceilP = floorP
                ceilL = floorL
        # value smaller than smallest element in the array
        elif result[1] == 2:
...
#        print "new segment b",start_b,end_b
    ...
    def get_tangent_at_arc_length(self, arc_length, eval_range=0.5):
        ...
        * start : np.ndarry
        ...
        magnitude = 0
        # handle cases where the granularity of the spline is too low
        while magnitude == 0:
...
# min_arc_length, max_arc_length)
        ...
        * distance : float
        ...
        if min_arc_length >= self.full_arc_length:
            ...
            segments = self._construct_segment_list(
                min_arc_length=min_arc_length, max_arc_length=max_arc_length)
    ...
    #        return closest_point,distance
            # min arc length was too close to full arc length
            if len(segments) == 0:
            ...
            if len(candidates) >= 2:
                closest_point_1, distance_1 = self._find_closest_point_on_segment(
                    point, candidates[0][1], accuracy, max_iterations)
                closest_point_2, distance_2 = self._find_closest_point_on_segment(
                    point, candidates[1][1], accuracy, max_iterations)
...
def plot_spline(spline, granularity=1000.0, ax=None):
    ...
    v, tangents = zip(* [(spline.query_point_by_parameter(u),
                          spline.get_angle_at_arc_length_2d(u * full_len, reference_vector)) for u in us])
