...
class MotionPrimitiveConstraintsBuilder(object):
    ...
    def build(self):
        ...
        mp_constraints.motion_primitive_name = self.status[
    ...
    def _set_trajectory_constraints(self, mp_constraints):
        ...
        # make a guess for a reachable point on the path that we have not
        # visited yet
    ...
    def _set_keyframe_constraints(self, mp_constraints):
        ...
        if self.status["motion_primitive_name"] in self.action_constraints.keyframe_constraints.keys():
            mp_constraints.constraints += self.action_constraints.keyframe_constraints[
    ...
    #    last_frame = prev_frames[-1]
        last_euler_frame = np.ravel(
            convert_quaternion_to_euler([self.status["prev_frames"][-1]]))
        position_dict = {}
    ...
    #        print "add joint position to constraints",node_name,joint_position
        ...
        frame_constraint = {"frame_constraint": position_dict, "semanticAnnotation": {
            "firstFrame": True, "lastFrame": None}}
    ...
    def _make_guess_for_goal_arc_length(self):
        ...
        * arc_length : float
        ...
        # find closest point in the range of the last_arc_length and
        # max_arc_length
        closest_point, distance = self.action_constraints.trajectory.find_closest_point(
            last_pos, min_arc_length=last_arc_length, max_arc_length=max_arc_length)
        if closest_point is None:
            parameters = {"last": last_arc_length, "max": max_arc_length,
                          "full": self.action_constraints.trajectory.full_arc_length}
        ...
        start_arc_length, eval_point = self.action_constraints.trajectory.get_absolute_arc_length_of_point(
            closest_point, min_arc_length=last_arc_length)
        # update arc length based on the step length of the next motion
        # primitive
        if start_arc_length == -1:
            return self.action_constraints.trajectory.full_arc_length
        else:
            # max(step_length-distance,0)
    ...
    def _get_point_and_orientation_from_arc_length(self, arc_length):
        ...
        """
        point = self.action_constraints.trajectory.query_point_by_absolute_arc_length(
            arc_length).tolist()
        reference_vector = np.array([0, 1])  # in z direction
        start, dir_vector, angle = self.action_constraints.trajectory.get_angle_at_arc_length_2d(
            arc_length, reference_vector)
