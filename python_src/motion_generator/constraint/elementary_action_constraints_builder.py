...
class ElementaryActionConstraintsBuilder():
    ...
    def build(self):
        ...
        action_constraints.action_name = self.motion_constraints.elementary_action_list[
            self.current_action_index]["action"]
        action_constraints.keyframe_annotations = self.motion_constraints.keyframe_annotations[
            self.current_action_index]
        action_constraints.constraints = self.motion_constraints.elementary_action_list[
            self.current_action_index]["constraints"]
        action_constraints.start_pose = self.motion_constraints.start_pose
        morphable_subgraph = self.motion_constraints.morphable_graph.subgraphs[
            action_constraints.action_name]
        # currently only trajectories on the Hips joint are supported
        root_joint_name = self.motion_constraints.morphable_graph.skeleton.root

        action_constraints.trajectory, action_constraints.unconstrained_indices = self._extract_trajectory_from_constraint_list(
            action_constraints.constraints, root_joint_name)
    ...
    def _extract_trajectory_from_constraint_list(self, constraint_list, joint_name):
        ...
        * unconstrained_indices: list of indices
        ...
        """
        trajectory_constraint = self._extract_trajectory_constraint(
            constraint_list, joint_name)
    ...
    def _create_trajectory_from_constraint(self, trajectory_constraint, scale_factor=1.0):
        ...
        * unconstrained_indices : list
        ...
        assert len(trajectory_constraint) > 0 and "position" in trajectory_constraint[
            0].keys()
        ...
        for v in trajectory_constraint[0]["position"]:
            ...
            idx += 1
        unconstrained_indices = transform_unconstrained_indices_from_cad_to_opengl_cs(
            unconstrained_indices)
        # create control points by setting constrained dimensions to 0
        control_points = []

        for c in trajectory_constraint:
            # else 0  where the array is None set it to 0
            point = [
                p * scale_factor if p is not None else 0 for p in c["position"]]
    ...
    def _extract_keyframe_constraint(self, joint_name, constraint, morphable_subgraph, time_information):
        ...
        * time_information : string
        ...
        constraint_desc = {"joint": joint_name, "position": position,
                           "orientation": orientation, "semanticAnnotation": semanticAnnotation}
    ...
    def _extract_keyframe_constraints_for_label(self, constraint_list, label):
        ...
        * key_constraints : dict of lists
        ...
        for c in constraint_list:
            ...
            # print c
            # there are keyframe constraints
            if "keyframeConstraints" in c.keys():
                ...
                for constraint_definition in c["keyframeConstraints"]:
                    ...
                    if self._constraint_definition_has_label(constraint_definition, label):
                        key_constraints[joint_name].append(
    ...
    def _extract_all_keyframe_constraints(self, constraint_list, morphable_subgraph):
        ...
        * keyframe_constraints : dict of dict of lists
        ...
        keyframe_constraints = {}
        # ["start_contact",]
        annotations = morphable_subgraph.annotation_map.keys()
        for label in annotations:
            #        print "extract constraints for annotation",label
            keyframe_constraints[label] = self._extract_keyframe_constraints_for_label(
                constraint_list, label)
    ...
    def _reorder_keyframe_constraints_for_motion_primitves(self, morphable_subgraph, keyframe_constraints):
        ...
        for label in keyframe_constraints.keys():
            ...
            time_information = morphable_subgraph.motion_primitive_annotations[
                state][label]
            constraints[state] = []
