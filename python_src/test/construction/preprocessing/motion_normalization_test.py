...
TESTDATAPATH = ROOTDIR + os.sep + r'../test_data/constrction/motion_normalization/cutting_results'
TESTRESULTPATH = ROOTDIR + os.sep + r'../test_output/constrction/motion_normalization'
...
class TestMotionNormalization(object):
    ...
    param_load_data_from_files_for_normalization = [{"data_folder": TESTDATAPATH}]
    
    @params(param_load_data_from_files_for_normalization)
    def test_load_data_from_files_for_normalization(self, data_folder):
        
        self.motion_normalizer.load_data_from_files_for_normalization(data_folder)
    ...
    def test_normalize_root(self, origin_point, touch_ground_joint, data_folder):
        ...
        assert self.motion_normalizer.ref_bvhreader.node_names['Hips']['offset'] == [0, 0, 0]
    ...
    def test_save_motion(self, data_folder, save_path):
        self.motion_normalizer.load_data_from_files_for_normalization(data_folder) 
        self.motion_normalizer.ref_bvhreader = BVHReader(self.motion_normalizer.ref_bvh)
