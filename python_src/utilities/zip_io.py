...
def construct_graph_data(zip_file, structure_desc, pickle_objects=False):
    ...
    for structure_key in structure_desc.keys():
        ...
        if meta_info_file in zip_file.namelist():
            graph_data[action_data_key]["info"] = json.loads(
                zip_file.read(meta_info_file))
        ...
        for mp in structure_desc[structure_key]:
            ...
            graph_data[action_data_key]["nodes"][
                mp_data_key]["name"] = mp[:-type_offset]
            ...
            statsfile = structure_key + "/" + \
                (mp[:-type_offset] + ".stats")  # .split("_")[1]
            ...
            if statsfile in zip_file.namelist():
                ...
                graph_data[action_data_key]["nodes"][
                    mp_data_key]["stats"] = stats_data
                # print
                # graph_data[action_data_key]["nodes"][mp_data_key]["stats"]

            space_partition_file = structure_key + \
                "/" + mp + "_cluster_tree.pck"

            if space_partition_file in zip_file.namelist():
                # print space_partition_file
                space_partition = zip_file.read(space_partition_file)
                graph_data[action_data_key]["nodes"][
                    mp_data_key]["space_partition"] = None
                if pickle_objects:
                    # space_partition_data =
                    # ClusterTree()#pickle.load(space_partition)
...
def read_graph_data_from_zip(zip_path, pickle_objects=True):
    ...
    with zipfile.ZipFile(zip_path, "r", zipfile.ZIP_DEFLATED) as zip_file:
        ...
        data["transitions"] = json.loads(
            zip_file.read("graph_definition.json"))
        structure_desc = read_zip_file_structure(zip_file)
        data["subgraphs"] = construct_graph_data(
            zip_file, structure_desc, pickle_objects)
    return data
