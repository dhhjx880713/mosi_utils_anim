...
class MGInputHandler(tornado.web.RequestHandler):
    ...
    def post(self):
        ...
        if "elementaryActions" in mg_input.keys():
            ...
            self._handle_result(
                mg_input, motion, self.application.use_file_output_mode, self.application.service_config)
        else:
            print mg_input
        ...
        if motion.quat_frames is not None:  # checks for quat_frames in result_tuple
            if use_file_output_mode:
                ...
                bvh_writer = get_bvh_writer(
                    self.application.motion_generator.morphable_graph.skeleton, motion.quat_frames)
                bvh_string = bvh_writer.generate_bvh_string()
                result_list = [
                    bvh_string, motion.frame_annotation, motion.action_list]
