        ...
        * filter_Yvalues: Bool
        ...
        quaternions = \
            self._get_all_nodes_quaternion_representation(
                bvh_reader, frame_number, filter_values)
        ...
        while current_axis < len(rotation_order):
            ...
            if rotation_order[current_axis] == 'X':
                ...
                if quaternion != 0:
                    ...
                    quaternion = temp_quaternion
            current_axis += 1
        # see http://physicsforgames.blogspot.de/2010/02/quaternions.html
        if filter_values:
            dot = quaternion.x + quaternion.y + quaternion.z + \
                quaternion.w  # dot product with [1,1,1,1]
            if dot < 0:  # flip
...
def test():
    from bvh import BVHReader
    filefolder = r'C:\Users\hadu01\MG++\repo\data\1 - MoCap\4 - Alignment\elementary_action_pick\first'
