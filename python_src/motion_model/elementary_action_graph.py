...
class ElementaryActionGraph(object):
    ...
    def __init__(self, skeleton_path, morphable_model_directory, transition_model_directory, load_transition_models=False, update_stats=False):
        ...
        * transition_model_directory: string
        ...
        if os.path.isfile(zip_path):
            self.init_from_zip_file(
                zip_path, transition_model_directory, load_transition_models)
        else:
            self.init_from_directory(morphable_model_directory, transition_model_directory,
    ...
    def init_from_zip_file(self, zip_path, transition_model_directory, load_transition_models):
        ...
        for el_action in subgraph_desc.keys():
            ...
            self.subgraphs[el_action].init_from_dict(subgraph_desc[
                                                     el_action], graph_definition, transition_model_directory, load_transition_models)
    ...
    def init_from_directory(self, morphable_model_directory, transition_model_directory, load_transition_models=False, update_stats=False):
        ...
        * transition_model_directory: string
        ...
        # load graphs representing elementary actions including transitions
        # between actions
        self.subgraphs = collections.OrderedDict()
        if morphable_model_directory is not None:
            ...
            for key in next(os.walk(morphable_model_directory))[1]:
                ...
                self.subgraphs[name].init_from_directory(
                    name, subgraph_path, transition_model_directory, load_transition_models, update_stats)

        graph_definition_file = morphable_model_directory + \
            os.sep + "graph_definition.json"
            ...
            for node_key in transition_dict:
                ...
                if from_action_name == subgraph_key and \
                   from_motion_primitive_name in self.subgraphs[from_action_name].nodes.keys():
                    # print "add action transitions for",
                    # subgraph_key,"###############################"
                        ...
                        if to_action_name != from_action_name:
                            ...
                            if load_transition_models and transition_model_directory is not None:
                                ...
                                if os.path.isfile(transition_model_file):
                                    output_gmm = self.subgraphs[to_action_name].nodes[
                            ...
                            self.subgraphs[from_action_name].nodes[
                                from_motion_primitive_name].outgoing_edges[to_key] = edge
...
def convert_graph_walk_to_euler_frames(skeleton, morphable_graph, graph_walk, node_name_map, smooth=True):
    ...
    * concatenated_frames: numpy.ndarray
    ...
    for entry in graph_walk:
        ...
        quaternion_frames = morphable_graph.subgraphs[subgraph].nodes[
            state].motion_primitive.back_project(parameters).get_motion_vector()
        euler_frames = convert_quaternion_to_euler(quaternion_frames.tolist())
        if concatenated_frames is not None:
            concatenated_frames = align_frames(
                skeleton, concatenated_frames, euler_frames, smooth)
        else:
...
def export_graph_walk_to_bvh(skeleton, graph_walk, morphable_graph, concatenate=True, apply_smoothing=True, prefix="", out_dir="."):
    ...
    if concatenate:
        ...
        filepath = out_dir + os.sep + prefix + "output_" + \
            unicode(datetime.datetime.now().strftime("%d%m%y_%H%M%S")) + ".bvh"
        BVHWriter(filepath, skeleton, concatenated_frames,
                  skeleton.frame_time, is_quaternion=False)
    else:
        time_code = out_dir + os.sep + prefix + "output_" + \
            unicode(datetime.datetime.now().strftime("%d%m%y_%H%M%S"))
        i = 0
        for entry in graph_walk:
            ...
            quaternion_frames = morphable_graph.subgraphs[subgraph].nodes[
                state].motion_primitive.back_project(parameters).get_motion_vector()
            euler_frames = convert_quaternion_to_euler(
                quaternion_frames.tolist())
            BVHWriter(filepath, skeleton, euler_frames,
                      skeleton.frame_time, is_quaternion=False)
            i += 1
...
def test_morphable_graph(load_transition_models=False):
    ...
    morphable_graph = ElementaryActionGraph(
        skeleton_file, mm_directory, transition_directory, load_transition_models)
    ...
    graph_walk = morphable_graph.generate_random_walk(
        elementary_action, n_steps, load_transition_models)
    export_graph_walk_to_bvh(morphable_graph.skeleton, graph_walk,
                             morphable_graph, True, True, prefix="smoothed_", out_dir="random_walks")
...
def load_subgraph(elementary_action="walk", load_transition_models=True):
    ...
    for key in next(os.walk(mm_directory))[1]:
        ...
        if name == elementary_action:
            ...
            subgraph.init_from_directory(
                name, subgraph_path, transition_directory, load_transition_models)
            break
...
def test_morphable_subgraph(elementary_action="walk", load_transition_models=True, apply_smoothing=True):
    ...
    if subgraph is not None:
        ...
        graph_walk = subgraph.generate_random_walk(
            start_state, n_steps, load_transition_models)
        print graph_walk
        ...
        export_graph_walk_to_bvh(skeleton, graph_walk, morphable_graph,
                                 True, apply_smoothing, prefix=prefix, out_dir="random_walks")
