...
#        print self.reshaped_fd.shape
        ...
        self.engenvectors = self.pcaobj.Vt[:self.pcaobj.npc]
        print 'number of eigenvectors: ' + str(self.pcaobj.npc)
        self.lowVs = self.project_fd(self.reshaped_fd)
        self.highVs = self.backproject_fd(self.lowVs)
    ...
    def project_fd(self, fd):
        '''
        project functional data to low dimensional space
        '''
        lowVs = []
        n_samples, n_dim = fd.shape
        for i in xrange(n_samples):
            lowV = np.dot(self.engenvectors, fd[i])
            lowVs.append(lowV)
        lowVs = np.asarray(lowVs)
        return lowVs
    
    def backproject_fd(self, lowVs):
        n_samples = len(lowVs)
        highVs = []
        for i in xrange(n_samples):
            highV = np.dot(np.transpose(self.engenvectors), lowVs[i].T)
