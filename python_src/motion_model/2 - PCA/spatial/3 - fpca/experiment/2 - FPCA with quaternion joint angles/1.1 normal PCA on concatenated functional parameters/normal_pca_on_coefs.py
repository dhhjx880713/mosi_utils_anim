...
# NOTE: USES THE OLD ANIMATIONDATA CLASS TO WRITE BVH FILES ONLY IN save_bvh()
from lib.quaternion_export.AnimationData import SkeletonAnimationData
from lib.quaternion_export.BVHWriter import BVHWriter

ROOT_DIR = os.sep.join([".."] * 7)

def get_input_maxdata_folder():
    """
...
def load_r_data(path):
    ...
    * path: string
    ...
    try:
        print path
        shutil.copyfile(path, 'functionalData.RData')
    except:
...
def load_max_data():
    path = clean_path(get_input_maxdata_folder()) + os.sep + 'walk_leftStance_maxVector.json'
...
def save_bvh(fd, filename):        
    eval_fd = robjects.r['eval.fd']
    t = robjects.r('c(0:46)')
    
    frames_r = eval_fd(t, fd)
    
    frames = np.array(frames_r)
    oldshape = frames.shape
    frames = np.reshape(frames, newshape=(oldshape[0], oldshape[-1]))
    
    maxima = load_max_data()
    frames[:, 0] *= maxima[0]
    frames[:, 1] *= maxima[1]
    frames[:, 2] *= maxima[2]
    
    # CRITICAL PART! HAS TO BE UPDATED ONCE A NEW BVH WRITER IS AVAILABLE!!!
    skeleton = os.sep.join(('lib', 'quaternion_export', 'skeleton.bvh'))
    anim = SkeletonAnimationData()
    anim.buildFromBVHFile(skeleton)
    anim.fromQuaternionVectorToMotionData(frames.flatten())
...
def calculate_errors(data, fraction, num_samples=620):
    ...
    * num_samples: int, optional
    ...
    for n in xrange(num_samples):
        ...
        error_n, originalfd =  compare_with_original(data, newfd, n)
        if errors is None:
            errors = error_n
        else:
    ...
    print "Mean error per dimension:"   
    print mean
    print "========================="
...
def save_new_and_old(data, pcaresult, basis, n):
    
    newfd = pca_backprojection(pcaresult, n, basis)
    
    # get original fd
    error_n, originalfd =  compare_with_original(data, newfd, n, plot=False)
    
    
    outputfolder = clean_path(get_output_data_folder()) + os.sep 
    save_bvh(newfd, outputfolder + "new_motion.bvh")
    save_bvh(originalfd, outputfolder + "original_motion.bvh")
...
def main():
    ...
    datapath = os.sep.join((path, 'walk_leftStance_functionalData.RData'))
    ...
    save_new_and_old(data, result, basis, sample_id) 
