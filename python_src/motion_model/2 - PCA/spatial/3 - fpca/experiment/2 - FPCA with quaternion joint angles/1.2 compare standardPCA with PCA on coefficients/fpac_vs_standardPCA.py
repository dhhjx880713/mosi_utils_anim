...
#    reconstructed_data = fpcaobj.reconstructed_data
#    print reconstructed_data[0,0,:]
#    print reconstructed_data.shape
    #################
    # reconstruct BVH files from motion data
    # the shape of reconstructed_data should be (n_samples, n_frames, n_dims)
#    n_samples, n_frames, n_dims = reconstructed_data.shape
#    skeleton = os.sep.join(('lib', 'skeleton.bvh'))
#    reader = BVHReader(skeleton)
#    for i in xrange(n_samples):
#        filename = 'generated_motions' + os.sep + str(i) + '.bvh'
#        frames = reconstructed_data[i,:,:]
#        BVHWriter(filename, reader, frames, frame_time=0.013889,
#                  is_quaternion=True)        
    #################
...
#    reconstructed_data = fpcaobj.reconstructed_data
#    print reconstructed_data.shape
#    plot_data(reconstructed_data, 6)
#    gen_motion_from_feature_vector(reconstructed_data)
#    print low_dimensional_functional_data.shape
#    print len(fileorder)
#     save load dimensional represetation as a dictionary
#    low_dimensional_functional_data = {}
#    for i in xrange(n_samples):
#        low_dimensional_functional_data[fileorder[i]] = fpcaobj.lowVs[i].tolist() 
#    print spatial_eigenvectors.shape
...
#    return fpcaobj.reconstructed_data                                         
    
#def test():
#    output_folder = get_output_folder()
#    print output_folder
#    filename = output_folder + os.sep + 'test.txt'
#    filename = clean_path(filename)
#    if len(filename) > 116:
#        filename = clean_path(filename)
##    path = r'C:\Users\hadu01\MG++\repo\data\3 - Motion primitives\motion_primitives_quaternion_PCA95\elementary_action_walk'
##    filename = path + os.sep + 'test.txt'    
#    with open(filename, 'wb') as outfile:
#        outfile.write('test')
#        outfile.close()
#    elementary_action = 'walk'
