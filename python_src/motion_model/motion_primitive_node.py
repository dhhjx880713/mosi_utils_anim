...
class MotionPrimitiveNode(object):
    ...
    * outgoing_edges: OrderedDict containing tuples
    ...
    def _construct_space_partition(self, cluster_file_name, reconstruct=False):
        # os.path.isfile(cluster_file_name+"tree.data") and
        # os.path.isfile(cluster_file_name+"tree.json"):
        if not reconstruct and os.path.isfile(cluster_file_name + "cluster_tree.pck"):
            ...
            self.cluster_tree.load_from_file_pickle(
            ...
            self.cluster_tree.save_to_file_pickle(
                cluster_file_name + "cluster_tree.pck")
    ...
    def generate_random_transition(self, transition_type=NODE_TYPE_STANDARD):
        ...
        * transition_type: string
        ...
        if self.outgoing_edges:
            edges = [e for e in self.outgoing_edges.keys() if self.outgoing_edges[
                e].transition_type == transition_type]
            if len(edges) > 0:
    ...
    def generate_random_action_transition(self, elementary_action):
        ...
        * elementary_action: string
        ...
        """
        if self.outgoing_edges:
            edges = [e for e in self.outgoing_edges.keys() if e.split(
                "_")[0] == elementary_action]
            if len(edges) > 0:
                random_index = random.randrange(0, len(edges), 1)
                to_key = edges[random_index]
                # print "to",to_key
    ...
    def update_attributes(self):
        ...
        n_samples = 50
        sample_lengths = [self._get_random_sample_step_length()
    ...
    def get_step_length_for_sample(self, s, method="arc_length"):
        ...
        *step_length: float
        ...
        quat_frames = self.motion_primitive.back_project(
            s, use_time_parameters=False).get_motion_vector()
        if method == "arc_length":
            ...
            step_length = np.linalg.norm(
                quat_frames[-1][:3] - quat_frames[0][:3])
        else:
    ...
    def predict_parameters(self, to_key, current_parameters):
        ...
        * next_parameters: numpy.ndarray
        ...
        """
        gmm = self.outgoing_edges[
            to_key].transition_model.predict(current_parameters)
