...
def create_directory_observer(path,pattern,callback=None,recursive=False):
    event_handler = CustomLoggingEventHandler(patterns=[pattern],callback=callback)
    logging.basicConfig(level=logging.INFO,
                        format='%(asctime)s - %(message)s',
                        datefmt='%Y-%m-%d %H:%M:%S')
...
class DirectoryObserverThread(threading.Thread):
    ...
    '''
    def __init__(self, path,pattern = "*.bvh",callback=None):
        threading.Thread.__init__(self)
        self.observer = create_directory_observer(path,pattern,callback)
