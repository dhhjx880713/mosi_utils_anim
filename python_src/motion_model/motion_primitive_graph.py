    ...
    * transition_model_directory: string
    ...
    def init_from_dict(self, subgraph_desc, graph_definition, transition_model_directory=None, load_transition_models=False):
        ...
        for m_primitive in subgraph_desc["nodes"].keys():
            ...
            self.nodes[m_primitive].init_from_dict(
                self.elementary_action_name, subgraph_desc["nodes"][m_primitive])

        if "info" in subgraph_desc.keys():
            self._set_meta_information(subgraph_desc["info"])
        else:
            self._set_meta_information()
        self._set_transitions_from_dict(
            graph_definition, transition_model_directory, load_transition_models)
    ...
    def init_from_directory(self, elementary_action_name, morphable_model_directory, transition_model_directory, load_transition_models=False, update_stats=False):
        ...
        # for files containing additional information that require the full
        # graph to be constructed first
        temp_file_list = []
        ...
        for root, dirs, files in os.walk(morphable_model_directory):
            for file_name in files:  # for each morphable model
                if file_name == "meta_information.json":
                    meta_information = load_json_file(
                        morphable_model_directory + os.sep + file_name)
                    ...
                    motion_primitive_file_name = morphable_model_directory + \
                        os.sep + file_name
                    self.nodes[motion_primitive_name] = MotionPrimitiveNode()
                    self.nodes[motion_primitive_name].init_from_file(
                        elementary_action_name, motion_primitive_name, motion_primitive_file_name)
        ...
        for file_name in temp_file_list:
            ...
            if motion_primitive in self.nodes.keys():
                info = load_json_file(
                    morphable_model_directory + os.sep + file_name, use_ordered_dict=True)
                self.nodes[motion_primitive].parameter_bb = info["pose_bb"]
                self.nodes[motion_primitive].cartesian_bb = info[
                    "cartesian_bb"]
                self.nodes[motion_primitive].velocity_data = info[
                    "pose_velocity"]

        self._set_transitions_from_directory(
            morphable_model_directory, transition_model_directory, load_transition_models)
    ...
    def _set_transitions_from_directory(self, morphable_model_directory, transition_model_directory, load_transition_models=False):
        ...
        if os.path.isfile(morphable_model_directory + os.sep + ".." + os.sep + "graph_definition.json"):
            graph_definition = load_json_file(
                morphable_model_directory + os.sep + ".." + os.sep + "graph_definition.json")
            self._set_transitions_from_dict(
                graph_definition, transition_model_directory, load_transition_models)
                    ...
                    if to_action_name == self.elementary_action_name:
                        ...
                        if transition_model_directory is not None and load_transition_models:
                            ...
                            if os.path.isfile(transition_model_file):
                                output_gmm = self.nodes[
                                    to_motion_primitive_name].motion_primitive.gmm
                        ...
                        self.nodes[from_motion_primitive_name].outgoing_edges[
                            to_key] = edge
        return
    ...
    def _set_meta_information(self, meta_information=None):
        ...
        if self.meta_information is None:
            self.start_states = [
                k for k in self.nodes.keys() if k.startswith("begin") or k == "first"]
            self.end_states = [
                k for k in self.nodes.keys() if k.startswith("end") or k == "second"]
        else:
            ...
            self.motion_primitive_annotations = self.meta_information[
                "annotations"]
            if "pattern_constraints" in self.meta_information.keys():
                pattern_constraints = self.meta_information[
                    "pattern_constraints"]
                for k in pattern_constraints.keys():
                    if k in self.nodes.keys():
                        self.nodes[
                            k].cluster_annotation = pattern_constraints[k]
            ...
            for motion_primitive in self.meta_information["annotations"].keys():
                if motion_primitive != "all_primitives":
                    motion_primitve_annotations = self.meta_information[
                        "annotations"][motion_primitive]
    ...
    def _update_attributes(self, update_stats=False):
        ...
        if update_stats:
            ...
            for k in self.nodes.keys():
                ...
                self.meta_information["stats"][k] = {"average_step_length": self.nodes[
                    k].average_step_length, "n_standard_transitions": self.nodes[k].n_standard_transitions}
            ...
            for k in self.nodes.keys():
                if k in self.meta_information["stats"].keys():
                    self.nodes[k].n_standard_transitions = self.meta_information[
                        "stats"][k]["n_standard_transitions"]
                    self.nodes[k].average_step_length = self.meta_information[
                        "stats"][k]["average_step_length"]
                else:
                    self.nodes[k].update_attributes()
                    self.meta_information["stats"][k] = {"average_step_length": self.nodes[
                        k].average_step_length, "n_standard_transitions": self.nodes[k].n_standard_transitions}
    ...
    def generate_random_walk(self, start_state, number_of_steps, use_transition_model=True):
        ...
        * use_transition_model: bool
        ...
        entry = {"subgraph": self.elementary_action_name,
                 "state": current_state, "parameters": current_parameters}
        ...
        if self.nodes[current_state].n_standard_transitions > 0:
            while count < number_of_steps:
                # sample transition
                # print current_state
                to_key = self.nodes[current_state].generate_random_transition(
                    NODE_TYPE_STANDARD)
                next_parameters = self.generate_next_parameters(
                    current_state, current_parameters, to_key, use_transition_model)
                # add entry to graph walk
                to_action = to_key.split("_")[0]
                to_motion_primitive = to_key.split("_")[1]
                entry = {"subgraph": to_action, "state":
                         to_motion_primitive, "parameters": next_parameters}
                graph_walk.append(entry)
        ...
        # add end state
        to_key = self.nodes[
            current_state].generate_random_transition(NODE_TYPE_END)
        next_parameters = self.generate_next_parameters(
            current_state, current_parameters, to_key, use_transition_model)
        to_action = to_key.split("_")[0]
        to_motion_primitive = to_key.split("_")[1]
        entry = {"subgraph": to_action, "state": to_motion_primitive,
    ...
    def generate_next_parameters(self, current_state, current_parameters, to_key, use_transition_model):
        ...
        * use_transition_model: bool
        ...
        if self.has_transition_models and use_transition_model:
            ...
            next_parameters = self.nodes[current_state].predict_parameters(
                to_key, current_parameters)
    ...
    def save_updated_meta_info(self):
        ...
        if self.meta_information is not None:
            path = self.morphable_model_directory + \
                os.sep + "meta_information.json"
    ...
    def get_canonical_keyframe_labels(self, motion_primitive_name):
        if motion_primitive_name in self.motion_primitive_annotations.keys():
            keyframe_labels = self.motion_primitive_annotations[
    ...
    def get_random_transition(self, motion, action_constraint, travelled_arc_length, arc_length_of_end):
        ...
        if action_constraint.trajectory is not None:
            ...
            n_standard_transitions = len([e for e in self.nodes[prev_mp_name].outgoing_edges.keys(
            ) if self.nodes[prev_mp_name].outgoing_edges[e].transition_type == NODE_TYPE_STANDARD])
        ...
        to_key = self.nodes[
            prev_mp_name].generate_random_transition(next_mp_type)
