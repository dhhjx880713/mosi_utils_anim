    ...
    *translation_maxima: numpy.ndarray
    ...
    def _init_spatial_parameters_from_json(self, data):
        ...
        * data: dictionary
        ...
        self.s_pca["knots"] = np.asarray(
            robjects.r['knots'](self.s_pca["basis_function"], False))
    ...
    def _init_time_parameters_from_json(self, data):
        ...
        * data: dictionary
        ...
        self.t_pca["knots"] = np.asarray(
            robjects.r['knots'](self.t_pca["basis_function"], False))
    ...
    def _inverse_temporal_pca(self, gamma):
        ...
        * time_function: numpy.ndarray
        ...
        eigen_tck = [(self.t_pca["knots"], self.t_pca["eigen_coefs"][i], 3)
                     for i in xrange(n_latent_dim)]
        eigen_t = np.array(
            [si.splev(self.canonical_time_range, tck) for tck in eigen_tck]).T

        t = [0, ]
        for i in xrange(self.n_canonical_frames):
            t.append(t[-1] + np.exp(mean_t[i] + np.dot(eigen_t[i], gamma)))

        # 1.2: undo step from timeVarinaces.transform_timefunction during
        # alignment
        t = np.array(t[1:])
        t -= 1
        zeroindices = t < 0
        ...
        # note this is where the number of frames are changed
