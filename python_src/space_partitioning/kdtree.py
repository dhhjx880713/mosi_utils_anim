...
class KDTree(object):
    ...
    def query(self, target, K=1):
        ...
        node_stack = []
        heapq.heappush(
            result_queue, (point_distance(self.root.point, target), self.root.point))
        node_stack.append(self.root)
        while len(node_stack) > 0:
            node = node_stack.pop(-1)
            best_option, least_distance = self._decide_direction_distance(
...
#                    bound = result_queue[K][0]
    ...
    def find_best_example(self, obj, data, k=1):
        ...
        while node is not None and node.type == "inner":
            ...
            node, least_cost = self._decide_direction_objective(
                node.left, node.right, obj, data)
            if node is not None:
