    ...
    * dim: Integer
    ...
    def construct_from_data(self, X, indices=None, depth=0):
        ...
        * depth: int
        ...
        # number of samples at least equal to the number of clusters required
        # for kmeans
        if n_samples > self.N:
            # create subdivision
            ...
            if depth < self.K:
                ...
                for j in xrange(len(cluster_indices)):
                    # if len(cluster_data[j]) > 0: #ignore clusters that are
                    # empty

                    if len(cluster_indices[j]) > 0:
                        child_node = self.construct_from_data(
                            X, cluster_indices[j], depth + 1)
        ...
        * X: np.ndarray
        ...
        if desc["type"] != LEAF_NODE:
            ...
            for c_id in desc["children"]:
                child_node = self.construct_from_node_desc_list(
                    c_id, node_desc, X)
                clusters.append(child_node)
        else:
            is_leaf = True
